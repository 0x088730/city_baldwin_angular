...
export class CareerComponent implements OnInit {
  ...
  constructor(private jobsStore: JobsStoreService, private jobInfoStore: JobsInfoStoreService, private departmentsService: DepartmentsStoreService) {
    this.sub = merge(
      this.departmentsService.data$.pipe(
      ...
      this.jobsStore.data$.pipe(
        tap(jobs => {
          jobs.map((job) => {
            job.department = this.departments?.find(department => department.id === job.department)?.title;
            return job;
          })
          this.jobs = jobs;
          this.result = jobs;
