...
export class ReportsComponent implements OnInit {
  ...
  openDialog(city) {
    ...
    this.dialog.open(ReportErrorDialogComponent, dialogConfig);
  }
  issue_report(e) {
    if (this.mapData.location) {
      ...
      this.http
      .get(
        environment.apiUrl + '/app/SIMT/tenants?latlng=' + this.mapData.location.lat + ',' + this.mapData.location.lng,
        options
        )
      .subscribe(
        (res) => {
          if(this.settings.address.toLowerCase().indexOf(res['mcs'].city) != -1) {
            console.log("This location is within the jurisdiction");
            ...
            if (this.is_imageId == true) {
              ...
              if (
                ...
                // });
                // const options = { headers };
                const reportUrl = environment.reportUrl + '/city/' + (environment.initTenant || this.city) + '/consumer';
            ...
            this.openDialog(this.settings.viewShortName)
            return false;
          }
        },
        (err) => {
